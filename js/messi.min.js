...
function Messi(e, t) {
	...
	if (n.options.title == null) {
		...
		if (n.options.titleClass != null)
			jQuery(".messi-titlebox", this.messi)
					.addClass(n.options.titleClass)
	}
	if (n.options.width != null)
		jQuery(".messi-box", n.messi).css("width", n.options.width);
	if (n.options.buttons.length > 0) {
		for ( var i = 0; i < n.options.buttons.length; i++) {
			var s = n.options.buttons[i]["class"] ? n.options.buttons[i]["class"]
					: "";
			var o = jQuery(
					'<div class="btnbox"><button class="btn ' + s
							+ '" href="#">' + n.options.buttons[i].label
							+ "</button></div>").data("value",
					n.options.buttons[i].val);
	...
	if (n.options.buttons.length === 0 && n.options.title == null
			&& !n.options.autoclose) {
		if (n.options.closeButton) {
			var r = jQuery('<span class="messi-closebtn"></span>');
			r.bind("click", function() {
...
Messi.prototype = {
	...
	show : function() {
		...
		if (this.options.center)
			this.options.viewport = this.viewport(jQuery(".messi-box",
					this.messi));
		this.messi.css({
	...
	hide : function(e) {
		...
		this.messi.animate({
			...
			if (t.options.modal && t.modal != null)
				t.modal.remove();
			t.messi.css({
				display : "none"
			}).remove();
		...
		jQuery(n)
				.load(
						function() {
							...
							var r = this.width > e.width
									|| this.height > e.height ? Math.min(
									e.width / this.width, e.height
											/ this.height) : 1;
							jQuery(n).css({
								width : this.width * r,
